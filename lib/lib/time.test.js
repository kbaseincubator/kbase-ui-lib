...
it('relative time, many permutations, compact', () => {
    ...
    testDataTable.forEach((testData) => {
        const result = time_1.niceDuration.apply(null, testData.args);
        expect(result).toEqual(testData.expected);
    });
});
it('renders a duration for 0', () => {
    const duration = 0;
    const result = time_1.niceDuration(duration);
...
it('renders a duration for 1 minute', () => {
    ...
    const result = time_1.niceDuration(duration, { precision: 1 });
...
it('renders an relative time for now', () => {
    ...
    const result = time_1.niceRelativeTime(refDate, { now: nowDate });
    expect(result).toEqual('now');
});
it('renders an relative time for 1 second from now', () => {
    const refDate = new Date(Date.now() + 1000);
    const result = time_1.niceRelativeTime(refDate);
...
it('renders an relative time for 2 seconds from now, compact', () => {
    const refDate = new Date(Date.now() + 2000);
    const result = time_1.niceRelativeTime(refDate, { compact: true });
...
it('renders many conditions, non-compact', () => {
    ...
    const testDataTable = [
        {
            args: [new Date(now - secMs)],
            expected: '1 second ago'
        },
        {
            args: [new Date(now - 2 * secMs)],
            expected: '2 seconds ago'
        },
        {
            args: [new Date(now - minMs)],
            expected: '1 minute ago'
        },
        {
            args: [new Date(now - 2 * minMs)],
            expected: '2 minutes ago'
        },
        {
            args: [new Date(now - hourMs)],
            expected: '1 hour ago'
        },
        {
            args: [new Date(now - 2 * hourMs)],
            expected: '2 hours ago'
        },
        {
            args: [new Date(now - dayMs)],
            expected: '1 day ago'
        },
        {
            args: [new Date(now - 2 * dayMs)],
            expected: '2 days ago'
        },
        {
            args: [new Date(now - secMs), { compact: true }],
            expected: '1s ago'
        },
        {
            args: [new Date(now - 2 * secMs), { compact: true }],
            expected: '2s ago'
        },
        {
            args: [new Date(now - minMs), { compact: true }],
            expected: '1m ago'
        },
        {
            args: [new Date(now - 2 * minMs), { compact: true }],
            expected: '2m ago'
        },
        {
            args: [new Date(now - hourMs), { compact: true }],
            expected: '1h ago'
        },
        {
            args: [new Date(now - 2 * hourMs), { compact: true }],
            expected: '2h ago'
        },
        {
            args: [new Date(now - dayMs), { compact: true }],
            expected: '1d ago'
        },
        {
            args: [new Date(now - 2 * dayMs), { compact: true }],
            expected: '2d ago'
        },
        {
            args: [new Date(now + secMs)],
    ...
    testDataTable.forEach((testData) => {
        const result = time_1.niceRelativeTime.apply(null, testData.args);
        expect(result).toEqual(testData.expected);
    });
});
...
it('renders an relative time beyond the limit', () => {
    ...
    const result = time_1.niceRelativeTime(refDate, { compact: true, now: nowDate });
    const expected = Intl.DateTimeFormat('en-US', {
        month: 'short',
        day: 'numeric'
    }).format(refDate);
